...
export const ArticleViews = ({ children }: { children: React.ReactNode }) => {
  ...
  switch (current) {
    case 'default':
      return (
        <div
          ...
          style={{
            transform: open && !isMobile ? 'translateX(calc(var(--sidebar-width) * -0.5))' : undefined,
          }}
        >
    ...
    case 'playground':
      return (
        <div className="p-1 flex-1 min-h-0 flex">
          <ResizablePanelGroup
            direction="horizontal"
            className="relative flex-1 overflow-hidden gap-0.5"
          >
