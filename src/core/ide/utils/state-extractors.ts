...
export const extractEditorContent = (state: IDEState): Record<string, string> => {
  ...
  // Extract content from file tree by finding file nodes
  const findNodeContent = (nodes: FileSystemNode[]): string | null => {
    for (const node of nodes) {
      if (node.type === 'file') {
        // Check if this file is in open tabs
        const isOpenTab = state.openTabs.some((tab) => tab.fileId === node.id)
        if (isOpenTab) {
          return node.content
        }
      }
  ...
  if (content) {
    // Map content to open tab file IDs
...
export const isEqual = (left: FileSystemState | null, right: FileSystemState | null): boolean => {
  ...
  for (const [id, leftNode] of leftTreeMap) {
    ...
    // Compare content and language for files
    ...
    } else if (leftNode.type === 'folder' && rightNode.type === 'folder') {
      // Compare children count for folders
      if (leftNode.children.length !== rightNode.children.length) return false
    } else {
      // If types don't match, or one is file and other is folder, they are not equal
      return false
    }
  }
